...
module('Integration | Component | evolution-chain', function (hooks) {
  ...
  test('renders the evolution line in order', async function (assert) {
    ...
    await render(<template><EvolutionChain @evolutionChainUrl="something" @currentId={{1}} /></template>);

     assert
      .dom('.evolution-name')
      .exists({ count: 3 }, 'renders all three stages');

    const names = this.element.querySelectorAll('.evolution-name');
    assert.strictEqual(names[0].textContent.trim(), 'bulbasaur');
    assert.strictEqual(names[1].textContent.trim(), 'ivysaur');
    assert.strictEqual(names[2].textContent.trim(), 'venusaur');
